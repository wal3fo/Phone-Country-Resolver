...
use Wal3fo\PhoneCountry\AI\FraudDetector;
use Wal3fo\PhoneCountry\AI\MetadataEnricher;
use Wal3fo\PhoneCountry\AI\NanpDisambiguator;
use Wal3fo\PhoneCountry\AI\PhoneExplainer;
use Wal3fo\PhoneCountry\AI\PhoneNormalizer;
...
class PhoneCountryService
{
    ...
    /**
     * Full AI-powered analysis of a phone number.
     *
     * Combines all AI features into a single PhoneAnalysis result:
     *   1. Smart normalization  — repairs messy/malformed inputs
     *   2. Resolution          — runs the standard prefix resolution
     *   3. NANP disambiguation — resolves +1 to the correct country
     *   4. Fraud detection     — scores risk and classifies number type
     *   5. Metadata enrichment — adds timezone, currency, language, length check
     *   6. Explanation         — generates a plain-language description
     *
     * @param  string  $phoneNumber      Raw phone number in any format.
     * @param  string  $localCountryCode ISO fallback for local 0xxx numbers.
     */
    public static function analyze(string $phoneNumber, string $localCountryCode = 'XX'): PhoneAnalysis
    {
        // 1. Smart normalization
        [$normalizedInput, $wasNormalized] = PhoneNormalizer::normalize($phoneNumber);

        // 2. Standard resolution
        $effectiveFallback = $localCountryCode !== 'XX'
            ? $localCountryCode
            : Config::get('phone-country.default_country', 'XX');

        $result = self::resolve($normalizedInput, $effectiveFallback);

        // 3. NANP disambiguation (only for +1 numbers)
        $cleanDigits            = preg_replace('/[^0-9]/', '', $normalizedInput);
        $disambiguatedCode      = $result->countryCode;
        $disambiguationNote     = null;

        if (NanpDisambiguator::isNanp($cleanDigits) && $result->countryCode === 'US') {
            $disambiguation     = NanpDisambiguator::disambiguate($cleanDigits);
            $disambiguatedCode  = $disambiguation['code'];
            $disambiguationNote = $disambiguation['note'];

            // If we found a more specific country, rebuild the result for fraud/meta
            if ($disambiguatedCode !== 'US') {
                $result = self::resolve($normalizedInput, $disambiguatedCode);
            }
        }

        // 4. Fraud detection
        $fraud = FraudDetector::analyze(
            $cleanDigits,
            $disambiguatedCode,
            $result->dialCode,
        );
        ...
        $explanation = PhoneExplainer::explain(
            result: $result,
            numberType: $fraud['type'],
        ...
        return new PhoneAnalysis(
            ...
            wasNormalized: $wasNormalized,
            disambiguatedCountryCode: $disambiguatedCode,
            disambiguationNote: $disambiguationNote,
            riskLevel: $fraud['level'],
            ...
            subscriberLength: $meta['subscriber_length'],
            lengthValid: $meta['length_valid'],
            explanation: $explanation,
        );
    }

    /**
     * Analyze multiple phone numbers at once.
     *
     * @param  string[]  $phoneNumbers
     * @return PhoneAnalysis[]
     */
    public static function analyzeMany(array $phoneNumbers, string $localCountryCode = 'XX'): array
    {
        return array_map(
            fn(string $number) => self::analyze($number, $localCountryCode),
            $phoneNumbers
        );
    }
